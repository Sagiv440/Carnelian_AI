...
/// <summary>
/// <see cref="IChatClient"/> over the OpenAI Chat Completions API (api.openai.com/v1). The API key is
/// read from settings on every call so the user can change it without a restart; a blank key reports
/// "not configured" (empty model list, unreachable) instead of erroring.
/// </summary>
public sealed class OpenAiClient : IOpenAiClient
{
    ...
    public OpenAiClient(HttpClient http, ISettingsService settings)
    {
        _http = http;
        _settings = settings;
    }
    ...
    private string ApiKey => _settings.Current.OpenAiApiKey.Trim();
    ...
    {
        // `think` is ignored: OpenAI exposes no chain-of-thought for the standard chat models.
    ...
    private static string BuildErrorMessage(int status, string body)
    {
        ...
        return $"OpenAI returned HTTP {status}: {detail}";
    }
}
